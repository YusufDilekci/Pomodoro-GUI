...
# ---------------------------- TIMER RESET ------------------------------- #
def reset_timer():
    window.after_cancel(timer) #yenilemeyi durdurmak için
...
def count_down(count):
    ...
    if count_sec < 10:
        count_sec = f'0{count_sec}'     #Dynamic typing it is private for python programming languagae changing data type
    canvas.itemconfig(timer_text, text=f'{count_min}:{count_sec}')
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count-1)  #bir nevi ekranın yenilenmesini sağlar
